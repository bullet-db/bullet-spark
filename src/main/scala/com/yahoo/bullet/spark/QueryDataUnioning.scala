...
object QueryDataUnioning {
  ...
  def receive(ssc: StreamingContext, broadcastedConfig: Broadcast[BulletSparkConfig]
             ): (DStream[(String, BulletData)], DStream[BulletRecord[_ <: Serializable]]) = {
  ...
  private def getBulletRecordStream(ssc: StreamingContext, config: BulletSparkConfig): DStream[BulletRecord[_ <: Serializable]] = {
